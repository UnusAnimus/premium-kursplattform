  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-[var(--text-secondary)]">Übersicht über die Arkanum Akademie</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        {metrics.map((m, i) => (
          <div key={i} className="bg-[var(--bg-surface)] border border-[var(--border-base)] rounded-xl p-5">
            ...
            <div className="text-[var(--text-secondary)] text-xs">{m.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-[var(--bg-surface)] border border-[var(--border-base)] rounded-xl overflow-hidden">
          <div className="flex items-center justify-between p-5 border-b border-[var(--border-base)]">
            <h2 className="text-lg font-semibold text-[var(--text-primary)]">Neueste Nutzer</h2>
            <Link href="/admin/nutzer" className="text-violet-400 hover:text-violet-300 text-sm">Alle →</Link>
          </div>
          {loading ? (
            <div className="p-8 text-center text-[var(--text-muted)] text-sm">Lädt…</div>
          ) : (stats?.recentUsers ?? []).length === 0 ? (
            <div className="p-8 text-center text-[var(--text-muted)] text-sm">Noch keine Nutzer registriert.</div>
          ) : (
        ...
        {/* Quick actions */}
        <div className="bg-[var(--bg-surface)] border border-[var(--border-base)] rounded-xl p-5">
          <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-5">Schnellaktionen</h2>
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <Link
                key={i}
                href={action.href}
                className="flex items-center gap-3 p-4 bg-[var(--bg-surface-raised)] rounded-xl hover:bg-[var(--bg-surface-hover)] transition-all border border-[var(--border-base)] hover:border-violet-500/50 group"
              >
                <span className="text-xl text-violet-400">{action.icon}</span>
                <span className="text-[var(--text-secondary)] text-sm group-hover:text-[var(--text-primary)]">{action.label}</span>
              </Link>
            ))}
          </div>
        </div>
