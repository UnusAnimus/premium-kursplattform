...
function TestimonialCard({ t }: { t: Testimonial }) {
  return (
    <div className="bg-[var(--bg-surface)] border border-[var(--border-base)] rounded-xl p-6 hover:border-violet-500/30 transition-all duration-300">
      <div className="flex items-center gap-1 mb-4">
        {[1, 2, 3, 4, 5].map(star => (
          <span key={star} className={star <= t.rating ? 'text-amber-400' : 'text-[var(--border-strong)]'}>★</span>
        ))}
      </div>
      <p className="text-[var(--text-secondary)] text-sm leading-relaxed mb-5">&ldquo;{t.content}&rdquo;</p>
      {t.courseTitle && (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 rounded-full px-4 py-2 mb-6">
            ...
            <span className="text-amber-400 text-sm font-medium">Was unsere Studierenden sagen</span>
          </div>
